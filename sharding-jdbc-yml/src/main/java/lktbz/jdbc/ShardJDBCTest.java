...
public class ShardJDBCTest {
    public static void main(String[] args) throws SQLException {
        ...
        dataSource1.setUrl("jdbc:mysql://localhost:3306/shard0?autoReconnect=true&useSSL=false&useAffectedRows=true&serverTimezone=Asia/Shanghai&characterEncoding=utf8");
        dataSource1.setUsername("root");
        dataSource1.setPassword("xiao");
        dataSourceMap.put("shard0", dataSource1);

        // 配置第二个数据源
        BasicDataSource dataSource2 = new BasicDataSource();
        dataSource2.setDriverClassName("com.mysql.cj.jdbc.Driver");
        dataSource2.setUrl("jdbc:mysql://localhost:3306/shard1?autoReconnect=true&useSSL=false&useAffectedRows=true&serverTimezone=Asia/Shanghai&characterEncoding=utf8");
        dataSource2.setUsername("root");
        dataSource2.setPassword("xiao");
        dataSourceMap.put("shard1", dataSource2);

        // 配置Order表规则
        TableRuleConfiguration orderTableRuleConfig = new TableRuleConfiguration();
        orderTableRuleConfig.setLogicTable("order");
        orderTableRuleConfig.setActualDataNodes("shard${0..1}.order${0..1}");


        // 配置分库 + 分表策略
        orderTableRuleConfig.setDatabaseShardingStrategyConfig(new InlineShardingStrategyConfiguration("order_id", "shard${order_id % 2}"));
